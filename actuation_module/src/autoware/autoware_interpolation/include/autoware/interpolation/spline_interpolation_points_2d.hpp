...
public:
  ...
  {
    std::vector<PointMsg> points_inner;
  ...
  PoseMsg getSplineInterpolatedPose(const size_t idx, const double s) const;

  // point
  PointMsg getSplineInterpolatedPoint(const size_t idx, const double s) const;
...
private:
  void calcSplineCoefficientsInner(const std::vector<PointMsg> & points);
