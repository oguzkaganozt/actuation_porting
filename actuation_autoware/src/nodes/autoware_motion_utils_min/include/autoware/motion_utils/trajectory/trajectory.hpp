...
extern template void validateNonEmpty<std::vector<PathPointMsg>>(
  const std::vector<PathPointMsg> &);
extern template void validateNonEmpty<std::vector<PathPointWithLaneIdMsg>>(
  const std::vector<PathPointWithLaneIdMsg> &);
extern template void validateNonEmpty<std::vector<TrajectoryPointMsg>>(
...
size_t findNearestIndex(const T & points, const PointMsg & point)
{
  ...
  validateNonEmpty(points_vec);

  double min_dist = std::numeric_limits<double>::max();
  size_t min_idx = 0;

  for (size_t i = 0; i < points_vec.size(); ++i) {
    const auto dist = autoware::universe_utils::calcSquaredDistance2d(points_vec.at(i), point);
    if (dist < min_dist) {
      min_dist = dist;
      min_idx = i;
    }
...
extern template size_t findNearestIndex<std::vector<PathPointMsg>>(
  const std::vector<PathPointMsg> & points,
  const PointMsg & point);
extern template size_t findNearestIndex<std::vector<PathPointWithLaneIdMsg>>(
  const std::vector<PathPointWithLaneIdMsg> & points,
  const PointMsg & point);
extern template size_t findNearestIndex<std::vector<TrajectoryPointMsg>>(
  ...
  const double max_yaw = std::numeric_limits<double>::max())
{
  ...
  for (size_t i = 0; i < points_vec.size(); ++i) {
    const auto squared_dist = autoware::universe_utils::calcSquaredDistance2d(points_vec.at(i), pose);
    if (squared_dist > max_squared_dist || squared_dist >= min_squared_dist) {
      continue;
    }

    const auto yaw = autoware::universe_utils::calcYawDeviation(
      autoware::universe_utils::getPose(points_vec.at(i)), pose);
...
//   if (points_vec.size() < start_idx + 1) {
//     return points;
//   }

//   T dst;
//   dst.reserve(points_vec.size());

//   for (size_t i = 0; i <= start_idx; ++i) {
//     dst.push_back(points_vec.at(i));
//   }

//   constexpr double eps = 1.0E-08;
//   for (size_t i = start_idx + 1; i < points_vec.size(); ++i) {
//     const auto prev_p = autoware::universe_utils::getPoint(dst.back());
//     const auto curr_p = autoware::universe_utils::getPoint(points_vec.at(i));
...
// extern template std::vector<PathPointMsg>
// removeOverlapPoints<std::vector<PathPointMsg>>(
//   const std::vector<PathPointMsg> & points, const size_t start_idx = 0);
// extern template std::vector<PathPointWithLaneIdMsg>
// removeOverlapPoints<std::vector<PathPointWithLaneIdMsg>>(
//   const std::vector<PathPointWithLaneIdMsg> & points,
//   const size_t start_idx = 0);
// extern template std::vector<TrajectoryPointMsg>
...
//   const auto overlap_removed_points = removeOverlapPoints(points_vec, seg_idx);
...
// extern template size_t findNearestSegmentIndex<std::vector<PathPointMsg>>(
//   const std::vector<PathPointMsg> & points,
//   const PointMsg & point);
...
//   const auto nearest_idx = findNearestIndex(points_vec, pose, max_dist, max_yaw);
...
//   const double signed_length = calcLongitudinalOffsetToSegment(points_vec, *nearest_idx, pose.position);
...
//   if (src_idx > dst_idx) {
//     return -calcSignedArcLength(points_vec, dst_idx, src_idx);
//   }

//   double dist_sum = 0.0;
//   for (size_t i = src_idx; i < dst_idx; ++i) {
//     dist_sum += autoware::universe_utils::calcDistance2d(points_vec.at(i), points_vec.at(i + 1));
//   }
//   return dist_sum;
// }

// extern template double calcSignedArcLength<std::vector<PathPointMsg>>(
//   const std::vector<PathPointMsg> & points, const size_t src_idx,
//   const size_t dst_idx);
// extern template double
// calcSignedArcLength<std::vector<PathPointWithLaneIdMsg>>(
//   const std::vector<PathPointWithLaneIdMsg> & points, const size_t src_idx,
//   const size_t dst_idx);
// extern template double
// calcSignedArcLength<std::vector<TrajectoryPointMsg>>(
...
//   const size_t src_seg_idx = findNearestSegmentIndex(points_vec, src_point);

//   const double signed_length_on_traj = calcSignedArcLength(points_vec, src_seg_idx, dst_idx);
//   const double signed_length_src_offset =
//     calcLongitudinalOffsetToSegment(points_vec, src_seg_idx, src_point);
...
//   std::vector<double> curvature_vec(points_vec.size(), 0.0);
//   if (points_vec.size() < 3) {
//     return curvature_vec;
//   }

//   for (size_t i = 1; i < points_vec.size() - 1; ++i) {
//     const auto p1 = autoware::universe_utils::getPoint(points_vec.at(i - 1));
//     const auto p2 = autoware::universe_utils::getPoint(points_vec.at(i));
//     const auto p3 = autoware::universe_utils::getPoint(points_vec.at(i + 1));
...
// calcCurvature<std::vector<PathPointMsg>>(
//   const std::vector<PathPointMsg> & points);
// extern template std::vector<double>
...
//   try {
//     validateNonEmpty(points_with_twist_vec);
//   } catch (const std::exception & e) {
//     LOG_ERR("Error: %s", e.what());
//     return {};
//   }
...
//   for (size_t i = src_idx; i < dst_idx; ++i) {
//     if (std::fabs(points_with_twist_vec.at(i).longitudinal_velocity_mps) < epsilon) {
//       return i;
//     }
//   }
...
//   return searchZeroVelocityIndex(points_with_twist_vec, src_idx, points_with_twist_vec.size());
// }
...
//   const auto & first_pose = autoware::universe_utils::getPose(points_vec.at(0));
//   const auto & second_pose = autoware::universe_utils::getPose(points_vec.at(1));
...
//         autoware::universe_utils::calcSquaredDistance2d(points_vec.at(i), pose.position);
//       const auto yaw = autoware::universe_utils::calcYawDeviation(
//         autoware::universe_utils::getPose(points_vec.at(i)), pose);
...
//   return findNearestIndex(points_vec, pose.position);
// }
...
//     findFirstNearestIndexWithSoftConstraints(points_vec, pose, dist_threshold, yaw_threshold);
