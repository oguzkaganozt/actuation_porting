...
// PORTING: WAITING FOR MESSAGE CONVERSION, WAITING FOR geometry.hpp to be ported
  ...
  const T & points, const geometry_msgs::msg::Point & src_point, const double offset)
{
  try {
    ...
    printf( "Failed to calculate longitudinal offset: %s", e.what());
    return {};
  }
  ...
  const bool set_orientation_from_position_direction = true, const bool throw_exception = false)
{
  ...
  if (points.size() == 1) {
    printf( "Failed to calculate longitudinal offset: points size is one.");
    return {};
  }
  ...
  const double max_yaw = std::numeric_limits<double>::max())
{
  try {
    ...
    printf( "Failed to calculate stop distance %s", e.what());
    return {};
  }
  ...
  const double forward_length, const double backward_length)
{
  ...
  if (cropped_points.size() < 2) {
    printf( "Return original points since cropped_points size is less than 2.");
    return points;
  }
